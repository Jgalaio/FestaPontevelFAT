...
add column if not exists fat_com_nif boolean not null default false;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'despesas_posto_tipo_pagamento_check'
      and conrelid = 'public.despesas_posto'::regclass
  ) then
    alter table public.despesas_posto
    add constraint despesas_posto_tipo_pagamento_check
    check (tipo_pagamento in ('dinheiro', 'transferencia'));
  end if;
end;
$$;

update public.despesas_posto
set fat_com_nif = coalesce(fat_com_nif, false),
    tipo_pagamento = case when tipo_pagamento = 'transferencia' then 'transferencia' else 'dinheiro' end;
...
despesas as (
  select
    ...
    sum(case when d.tipo_pagamento = 'dinheiro' then d.valor else 0 end) as despesas_dinheiro,
    sum(case when d.tipo_pagamento = 'transferencia' then d.valor else 0 end) as despesas_transferencia
  from public.despesas_posto d
  group by d.data
)
select
  ...
  coalesce(despesas.despesas_dinheiro, 0) as despesas_dinheiro,
  coalesce(despesas.despesas_transferencia, 0) as despesas_transferencia
from faturacao
